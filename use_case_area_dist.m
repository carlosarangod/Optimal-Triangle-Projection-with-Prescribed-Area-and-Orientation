...
dist_const_flag = true;
pin_const_flag = false;
...
if start_flag
    ...
    Prev_X = X;
    Xtemp = X;
    fixed_seg_left = Sides{3};
    fixed_seg_right = Sides{4};    
    fixed_indices = cat(2,fixed_seg_left,fixed_seg_right);
    Norm_vertex = zeros(length(fixed_indices),2);
    Norm_vertex(1,1) = 1;
    Norm_vertex(end,1) = -1;
    X_diff = Xtemp(fixed_indices(1:end-2),:) - Xtemp(fixed_indices(3:end),:);
    Norm_vertex(2:end-1,1) = -X_diff(:,2);
    Norm_vertex(2:end-1,2) = X_diff(:,1);
    Norm_vertex = Norm_vertex./vecnorm(Norm_vertex,2,2);
    Xtemp(fixed_indices,:) = Xtemp(fixed_indices,:) +0.3*Norm_vertex;

%     X_norm = X_diff
%     Xtemp(fixed_seg_left,2) = Xtemp(fixed_seg_left,2) -0.2;
%     Xtemp(fixed_seg_right,2) = Xtemp(fixed_seg_right,2) - 0.2;
    iter = 30;
    oldPos = X(fixed_indices,:);
    mid_Step = (Xtemp(fixed_indices,:)-oldPos)/iter;
    newPos = oldPos;
    figure(f1)
    for k = 1:length(fixed_indices)
        circ(k) = plot(X(fixed_indices(k),1),X(fixed_indices(k),2),'o','MarkerSize',10,'MarkerFaceColor',red,...
            'MarkerEdgeColor',red,'UserData',k,'Tag',strcat('circ',num2str(k)));
    end

    for k=1:iter
        X(fixed_indices,:) = X(fixed_indices,:) + mid_Step;
        set(circ,'XData', X(fixed_indices,1));
        set(circ,'YData', X(fixed_indices,2));
        figure(1)
        delete(trplt1)
        trplt1 = triplot(DT,X(:,1),X(:,2));
        h = get(gca,'Children');
    ...
    if start_flag
        ...
        if pbd_lin_flag; a2 = plot(Area_hist2,'LineWidth',2); end
        xlabel('# iterations'); ylabel('Constraint Satisfaction');
        title('Constraint Satisfaction')
        if pbd_lin_flag
            h2 = legend('PBD-opt','PBD-lin');
        else
            h2 = legend('PBD-opt');
        end
        ...
        figure(f4)
        cla; hold on;
        plot(Disp_hist1,'LineWidth',2);
        if pbd_lin_flag; plot(Disp_hist2,'LineWidth',2); end
